...
import {Py_List, Py_Tuple} from './collections';
...
export class Thread {
    ...
    public id: number = -1;
    // SETUP_EXCEPT blockstack, used to catch the StopIteration exception generated by exhausted generators inside of for loops
    public loop_exc_block: [number,number,number,number][] = [];
    public tb: Py_Traceback = new Py_Traceback();
    ...
    // Get stack trace of function calls from wherever this function is invoked
    public getStackTrace(): [string, string, string, string][]  {
        var x: [string, string, string, string][] = [];
        var f: IPy_FrameObj;
        for (var i = this.stack.length - 1; i >= 0; i--) {
            f = this.stack[i];
            var tmp = f.getStackContents(this);
            if(tmp) {
                x.push(tmp);
            }
        }
